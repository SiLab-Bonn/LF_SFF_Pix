...
def calibrate_ADC():
    ...
    for adc_ch in ch_list:    
        ...
        for V in voltages:
            ...
            plt.plot(meas,np.linspace(offset+np.min(voltages), np.max(voltages)+offset,len(meas)), alpha=0.3, label=str(np.round(input_data[ch_list.index(adc_ch)][voltages.index(V)],3))+'V')
